...
empresas.get('/:id', async (c) => {
  const row = await c.env.DB.prepare('SELECT * FROM empresas WHERE id = ? LIMIT 1').bind(c.req.param('id')).first();
...
empresas.post('/', requireRole('admin'), async (c) => {
  const body = await c.req.json<{
    nombre: string; nit?: string; logo_url?: string; responsable_sg_sst?: string;
    correo?: string; telefono?: string; ciudad?: string;
  }>();
  ...
  await c.env.DB.prepare(`
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?)
  `).bind(id, body.nombre, body.nit??null, body.logo_url??null, body.responsable_sg_sst??null, body.correo??null, body.telefono??null, body.ciudad??null, now, now).run();
...
empresas.put('/:id', requireRole('admin'), async (c) => {
  ...
  `).bind(body.nombre, body.nit??null, body.logo_url??null, body.responsable_sg_sst??null, body.correo??null, body.telefono??null, body.ciudad??null, now, c.req.param('id')).run();
