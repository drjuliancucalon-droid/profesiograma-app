...
historial.get('/', async (c) => {
  ...
  const { results } = await c.env.DB
    .prepare('SELECT * FROM historial_versiones WHERE profesiograma_id = ? AND organizacion_id = ? ORDER BY version DESC')
    .bind(profesiogramaId, orgId(c)).all();
  return c.json({ success: true, data: results });
});

historial.get('/:id', async (c) => {
  const row = await c.env.DB
    .prepare('SELECT * FROM historial_versiones WHERE id = ? AND organizacion_id = ? LIMIT 1')
